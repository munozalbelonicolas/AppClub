...
class FirestoreService {
  ...
  Stream<List<Map<String, dynamic>>> getAnnouncementsForUser(String? category, bool isAdmin) {
    return _db
        .collection('announcements')
        .snapshots()
        .map((snapshot) {
          ...
          return list.where((ann) {
            ...
            if (cat == 'todos' || cat == 'all' || cat == 'general' || cat == 'deportivo' || cat == 'administrativo') {
              return true;
            }
  ...
  Future<void> addCommentToAnnouncement(String announcementId, Map<String, dynamic> commentData) async {
  ...
  Future<void> deleteCommentFromAnnouncement(String announcementId, Map<String, dynamic> commentData) async {
    await _db.collection('announcements').doc(announcementId).update({
  ...
  Future<void> addCommentToNovedad(String novedadId, Map<String, dynamic> commentData) async {
  ...
  Future<void> deleteCommentFromNovedad(String novedadId, Map<String, dynamic> commentData) async {
    await _db.collection('novedades').doc(novedadId).update({
      'comments': FieldValue.arrayRemove([commentData]),
    });
  }
...
final userAnnouncementsStreamProvider = StreamProvider.family<List<Map<String, dynamic>>, UserAnnouncementQuery>((ref, query) {
  return ref.watch(firestoreServiceProvider).getAnnouncementsForUser(query.category, query.isAdmin);
});
...
final userNovedadesStreamProvider = StreamProvider.family<List<Map<String, dynamic>>, String?>((ref, category) {
